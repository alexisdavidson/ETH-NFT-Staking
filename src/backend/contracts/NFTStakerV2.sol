...
contract NFTStakerV2 is ERC721Holder, MyOwnable, ReentrancyGuard {
    ...
    function unstake(uint256[] memory _tokenIds) public nonReentrant {
        for(uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            if (placeholderNft.ownerOf(stakers[msg.sender].placeholderTokenIds[_tokenIndex]) == msg.sender) {
                placeholderNft.safeTransferFrom(
                    msg.sender, 0x000000000000000000000000000000000000dEaD, stakers[msg.sender].placeholderTokenIds[_tokenIndex]);
            }
    ...
    function removeStakerElement(address _user, uint256 _tokenIndex, uint256 _lastIndex) internal {
        ...
        stakers[_user].placeholderTokenIds[_tokenIndex] = stakers[_user].placeholderTokenIds[_lastIndex];
